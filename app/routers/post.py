...
def create_posts(post: schemas.CreatePost, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    # cursor.execute("""INSERT INTO posts (title, contents, published) VALUES (%s, %s, %s) RETURNING *""",(post.title, post.content, post.published) )
...
def update_posts(id: int, updated_post: schemas.CreatePost, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    # cursor.execute("""UPDATE posts SET title = %s, contents = %s, published = %s WHERE id = %s RETURNING *""",(post.title, post.content, post.published, str(id)))
